...
var (
    ...
    de     = []byte("dhULNVGsuAk/MxH6ibjcEfRqDWYznXBe9Pl7+SKoZ8pJaICgrQO0mF21yv345wtT")
    en_map = make(map[byte]byte)
    de_map = make(map[byte]byte)

    neoreg_hello = []byte("6UNI/jhLR7X7fqPmY+m0BofOMNXNbVV2XNbiEVEODRxUbshHWKXC/mQWx0SNYVDFx1bKY0VDjcS3RcS/nGIOzVA0XOdI/cy=")
...
func blv_encode(info map[int][]byte) []byte {
    ...
    for b, v := range info {
        ...
        binary.Write(data, binary.BigEndian, int32(l) + 1966546385)
